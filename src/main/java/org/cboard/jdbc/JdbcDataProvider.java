...
public class JdbcDataProvider extends DataProvider implements Aggregatable, Initializing {
	...
	private Function<DimensionConfig, String> filter2SqlCondtion = (config) -> {
		...
		case "LIKE":
			...
			if (null != values && values.size() > 0) {
				for (int i = 0; i < values.size(); i++) {
					...
					str += config.getColumnName() + " LIKE " + v;
					if (i < values.size() - 1) {
						str += " AND ";
					}
				}
		...
		case "(a,b]":
			if (config.getValues().size() >= 2) {
				return "(" + config.getColumnName() + " > '" + dimensionConfigHelper.getValueStr(config, 0) + "' AND "
						+ config.getColumnName() + " <= " + dimensionConfigHelper.getValueStr(config, 1) + ")";
			} else {
				return null;
			}
		case "[a,b)":
	...
	private String assembleSqlFilter(Stream<ConfigComponent> filterStream, String prefix) {
		...
		where.setEmptyValue("");
		filterStream.map(e -> separateNull(e)).map(e -> configComponentToSql(e)).filter(e -> e != null)
				.forEach(where::add);
